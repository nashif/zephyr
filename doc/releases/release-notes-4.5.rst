...
* Bluetooth
  ...
  * Host
    ...
    * The ``CONFIG_BT_RECV_CONTEXT`` choice and its options ``CONFIG_BT_RECV_WORKQ_SYS``
      ...
      processes low-priority HCI packets and its internal work items on the
      dedicated Bluetooth RX workqueue (the former ``CONFIG_BT_RECV_WORKQ_BT``
      behavior). See the migration guide.
