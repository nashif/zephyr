...
* The driver is expected to be able to "announce" new ticks to the
  kernel via the ``clock_announce()`` call, which passes an integer
...
* The ``clock_announce()`` call is expected to be globally
...
* The next timeout value passed back to the driver via
  :c:func:`clock_set_timeout` is done identically for every CPU.
  ...
  non-zero ticks argument to ``clock_announce()``.  This is probably
...
value passed to :c:func:`clock_set_timeout` may be clamped to a
