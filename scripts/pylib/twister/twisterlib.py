...
{Fore.RED}{results.total - run - results.skipped_configs}{Fore.RESET} test configurations were only built.")
    ...
    def apply_filters(self, **kwargs):
        ...
        for scenario_name, scenario in self.scenarios.items():
            ...
            if default_platforms and not scenario.build_on_all:
                if scenario.platform_allow:
                    ...
                    instances = list(filter(lambda i: i.platform.default, instance_list))
                    if self.integration:
                        instances += list(filter(lambda item: item.platform.name in scenario.integration_platforms, \
                                         instance_list))
