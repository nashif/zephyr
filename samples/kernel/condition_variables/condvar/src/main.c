...
void inc_count(void *p1, void *p2, void *p3)
{
	...
	for (i = 0; i < TCOUNT; i++) {
		...
		if (count == COUNT_LIMIT) {
			printk("inc_count(): thread %ld, count = %d  Threshold reached. ",
			       my_id, count);
			k_condvar_signal(&count_threshold_cv);
			printk("Just sent signal.\n");
		}
		printk("inc_count(): thread %ld, count = %d, unlocking mutex\n",
		       my_id, count);
...
void watch_count(void *p1, void *p2, void *p3)
{
	...
	printk("Starting watch_count(): thread %ld\n", my_id);

	k_mutex_lock(&count_mutex, K_FOREVER);
	while (count < COUNT_LIMIT) {
		printk("watch_count(): thread %ld Count= %d. Going into wait...\n",
		       my_id, count);
		k_condvar_wait(&count_threshold_cv, &count_mutex, K_FOREVER);

		printk("watch_count(): thread %ld Condition signal received. Count= %d\n",
		       my_id, count);
	}
	printk("watch_count(): thread %ld Updating the value of count...\n",
	       my_id);
	count += 125;
	printk("watch_count(): thread %ld count now = %d.\n", my_id, count);
	printk("watch_count(): thread %ld Unlocking mutex.\n", my_id);
	k_mutex_unlock(&count_mutex);
}
