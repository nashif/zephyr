...
/* Cycles left to process in the currently-executing clock_announce() */
...
{
	return announce_remaining == 0 ? clock_elapsed() : 0U;
}
	...
	LOCKED(&timeout_lock) {
		...
		if (to == first()) {
			...
			if (next_time == 0 ||
				...
				clock_set_timeout(next_time, false);
			}
#else
			clock_set_timeout(next_timeout(), false);
...
void z_set_timeout_expiry(int32_t ticks, bool is_idle)
{
	LOCKED(&timeout_lock) {
		...
		if (!imminent && (sooner || IS_ENABLED(CONFIG_SMP))) {
			clock_set_timeout(MIN(ticks, next_to), is_idle);
		}
	}
}

void clock_announce(int32_t ticks)
{
#ifdef CONFIG_TIMESLICING
	z_time_slice(ticks);
#endif
