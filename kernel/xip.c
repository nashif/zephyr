...
void z_data_copy(void)
{
	...
	k_priv_early_memcpy(&__ramfunc_region_start, &__ramfunc_load_start,
		       __ramfunc_end - __ramfunc_region_start);
#endif /* CONFIG_ARCH_HAS_RAMFUNC_SUPPORT */
#ifdef CONFIG_ARCH_HAS_NOCACHE_MEMORY_SUPPORT
#if CONFIG_NOCACHE_MEMORY
	k_priv_early_memcpy(&_nocache_ram_start, &_nocache_load_start,
	...
	k_priv_early_memcpy(&__ccm_data_start, &__ccm_data_load_start,
		       __ccm_data_end - __ccm_data_start);
#endif
#if DT_NODE_HAS_STATUS_OKAY(DT_CHOSEN(zephyr_itcm))
	k_priv_early_memcpy(&__itcm_start, &__itcm_load_start,
		       (uintptr_t) &__itcm_size);
#endif
#if DT_NODE_HAS_STATUS_OKAY(DT_CHOSEN(zephyr_dtcm))
	k_priv_early_memcpy(&__dtcm_data_start, &__dtcm_data_load_start,
...
#else
	k_priv_early_memcpy(&_app_smem_start, &_app_smem_rom_start,
