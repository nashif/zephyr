...
int z_impl_k_mutex_init(struct k_mutex *mutex)
{
	...
	CHECK(mutex->owner != NULL && mutex->owner == _current) {
		ret = -EBUSY;
		goto out;
	}
...
int z_impl_k_mutex_reset(struct k_mutex *mutex)
{
	int ret = 0;
...
#ifdef CONFIG_USERSPACE
static inline int z_vrfy_k_mutex_reset(struct k_mutex *mutex)
{
	Z_OOPS(Z_SYSCALL_OBJ_INIT(mutex, K_OBJ_MUTEX));
	return z_impl_k_mutex_reset(mutex);
}
#include <syscalls/k_mutex_reset_mrsh.c>
#endif
