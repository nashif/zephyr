 ...
 *
 * With @kconfig{CONFIG_RING_BUFFER} enabled, the deprecated claim/finish and
 * item APIs remain available on top of the same implementation, at the cost
 * of two extra index fields per ring buffer.
 *
 * @{
 */
...
struct ring_buf {
	...
	/* Bytes covered by outstanding ring_buf_put_claim()/ring_buf_get_claim() calls */
	ring_buf_idx_t put_pending;
	ring_buf_idx_t get_pending;
#endif
	/** @endcond */
};
...
/*
 * Force both indices to @p value, which must be below twice the capacity.
 * Any value other than 0 only makes sense in validation tests.
 */
static inline void ring_buf_internal_reset(struct ring_buf *rb, ring_buf_idx_t value)
{
	__ASSERT_NO_MSG(value < 2U * rb->size);
 ...
 *         buffer.
 */
static inline uint32_t ring_buf_get_ptr(const struct ring_buf *rb, uint8_t **data, size_t offset)
{
...
/*
 * Deprecated claim/finish and item APIs, implemented on top of the
 * ring_buf_put_ptr()/ring_buf_commit() and ring_buf_get_ptr()/ring_buf_consume()
 * primitives. The pending counters in struct ring_buf record how many bytes
 * the outstanding claims on each side cover.
 */
 ...
 * @code extern struct ring_buf <name>; @endcode
 *
 * @deprecated Use @ref SYS_RINGQ_DEFINE from <zephyr/sys/ringq.h> instead.
 *
 * @param name Name of the ring buffer.
...
#define RING_BUF_ITEM_DECLARE(name, size32)                                                        \
	__DEPRECATED_MACRO                                                                         \
	BUILD_ASSERT((size32) <= RING_BUFFER_MAX_SIZE / 4U, RING_BUFFER_SIZE_ASSERT_MSG);          \
	static uint32_t __noinit _ring_buffer_data_##name[(size32)];                               \
	struct ring_buf name = RING_BUF_INIT((uint8_t *)_ring_buffer_data_##name, 4U * (size32))
 ...
 *
 * @deprecated Use <zephyr/sys/ringq.h> instead.
 *
 ...
 */
__deprecated static inline void ring_buf_item_init(struct ring_buf *rb, uint32_t size,
						   uint32_t *data)
{
	__ASSERT(size <= RING_BUFFER_MAX_SIZE / 4U, RING_BUFFER_SIZE_ASSERT_MSG);
	ring_buf_init(rb, 4U * size, (uint8_t *)data);
}
 ...
 */
__deprecated static inline uint32_t ring_buf_item_space_get(const struct ring_buf *rb)
{
	return ring_buf_space_get(rb) / 4U;
}
 ...
 * Successive claims stack until @ref ring_buf_put_finish is called.
 *
 * @warning
 ...
 *
 * @deprecated Use @ref ring_buf_put_ptr and @ref ring_buf_commit instead.
 *
 * @param[in]  rb   Address of ring buffer.
 ...
 */
__deprecated static inline uint32_t ring_buf_put_claim(struct ring_buf *rb, uint8_t **data,
						       uint32_t size)
{
	uint32_t avail = ring_buf_put_ptr(rb, data, rb->put_pending);

	size = MIN(size, avail);
	rb->put_pending = (ring_buf_idx_t)(rb->put_pending + size);
 ...
 * @retval -EINVAL Provided @a size exceeds the claimed space.
 */
__deprecated static inline int ring_buf_put_finish(struct ring_buf *rb, uint32_t size)
{
	if (size > rb->put_pending) {
		return -EINVAL;
	}
 ...
 * using @ref ring_buf_get_finish. Successive claims stack until
 * @ref ring_buf_get_finish is called.
 *
 * @warning
 ...
 * (calls prefixed with ring_buf_item_).
 *
 * @deprecated Use @ref ring_buf_get_ptr and @ref ring_buf_consume instead.
 *
 * @param[in]  rb   Address of ring buffer.
 * @param[out] data Pointer to the address. It is set to a location within
 *		    ring buffer.
 ...
 */
__deprecated static inline uint32_t ring_buf_get_claim(struct ring_buf *rb, uint8_t **data,
						       uint32_t size)
{
	uint32_t avail = ring_buf_get_ptr(rb, data, rb->get_pending);

	size = MIN(size, avail);
	rb->get_pending = (ring_buf_idx_t)(rb->get_pending + size);
 ...
 *
 * @retval 0 Successful operation.
 * @retval -EINVAL Provided @a size exceeds the claimed data.
 */
__deprecated static inline int ring_buf_get_finish(struct ring_buf *rb, uint32_t size)
{
	if (size > rb->get_pending) {
		return -EINVAL;
	}
 ...
 *
 * This routine writes a data item to ring buffer @a rb. The data item
 ...
 */
__deprecated int ring_buf_item_put(struct ring_buf *rb, uint16_t type, uint8_t value,
				   uint32_t *data, uint8_t size32);

/**
 * @brief Read a data item from a ring buffer.
 *
 * This routine reads a data item from ring buffer @a rb. The data item
 ...
 */
__deprecated int ring_buf_item_get(struct ring_buf *rb, uint16_t *type, uint8_t *value,
				   uint32_t *data, uint8_t *size32);
