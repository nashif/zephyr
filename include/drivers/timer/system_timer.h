 ...
 */
extern int clock_driver_init(const struct device *device);
 ...
 * clock_announce() will not be until the specified number of ticks
 * from the the current time have elapsed.  Note that spurious calls
 * to clock_announce() are allowed (i.e. it's legal to announce
 ...
 * clock_announce() may occur at any point in the future, not just
 ...
 * The resulting calls(s) to clock_announce() must be properly
 ...
 */
extern void clock_set_timeout(int32_t ticks, bool idle);
 ...
 */
extern void clock_announce(int32_t ticks);

/**
 * @brief Ticks elapsed since last clock_announce() call
 *
 * Queries the clock driver for the current time elapsed since the
 * last call to clock_announce() was made.  The kernel will call
 * this with appropriate locking, the driver needs only provide an
 * instantaneous answer.
 */
extern uint32_t clock_elapsed(void);
/**
 * @}
 */
