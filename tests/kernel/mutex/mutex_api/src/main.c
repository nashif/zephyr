...
{
	zassert_true(k_mutex_lock((struct k_mutex *)p1, TIMEOUT - 100) != 0,
		     NULL);
	TC_PRINT("bypass locked resource from spawn thread\n");
}
...
{
	k_mutex_init(pmutex);
	k_tid_t tid = k_thread_create(&tdata, tstack, STACK_SIZE, entry_fn,
				      pmutex, NULL, NULL, K_PRIO_PREEMPT(0),
...
void test_main(void)
{
	k_thread_access_grant(k_current_get(), &mutex, &kmutex, &tdata,
			      &tstack);

	ztest_test_suite(
		mutex_api, ztest_user_unit_test(test_mutex_lock_unlock),
