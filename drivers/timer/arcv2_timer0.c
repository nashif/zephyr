 ...
 * and it is updated in z_timer_int_handler and clock_set_timeout().
 *
 * Note:
...
static void timer_int_handler(const void *unused)
{
	...
	clock_announce(dticks);
#else
	/* timer_int_handler may be triggered by timer irq or
	 * software helper irq
	 */

	/* irq with higher priority may call clock_set_timeout
	 * so need a lock here
	 */
	uint32_t key;
	...
	clock_announce(TICKLESS ? dticks : 1);
#endif
