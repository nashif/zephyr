...
#include <device.h>
#include <watchdog.h>
#include <watchdog/qmsi_watchdog.h>
...
static void get_config(struct wdt_config *cfg)
{
	...
	/* Translate timeout from QMSI format to milliseconds. */
...
static int set_config(struct wdt_config *cfg)
{
	...
	/* Translate cfg->timeout (in milliseconds) to QMSI timeout format
...
static struct wdt_driver_api api = {
	.enable = qm_wdt_start,
	/* QMSI has no disable API for watchdog so we set the disable callback
	 * to a dummy function.
	 */
	.disable = dummy,
...
int qmsi_wdt_init(struct device *dev)
{
