 ...
 * cached one via sys_cache_cached_ptr(). However, physical addresses
 * are untouched.
 */
...
int sys_mm_drv_map_page(void *virt, uintptr_t phys, uint32_t flags)
{
	 ...
	 * bound check.
	 */
	uintptr_t pa = POINTER_TO_UINT(sys_cache_cached_ptr(UINT_TO_POINTER(phys)));
	uintptr_t va = POINTER_TO_UINT(sys_cache_cached_ptr(virt));
	...
	if (UINT_TO_POINTER(phys) == NULL) {
		...
		pa = POINTER_TO_UINT(sys_cache_cached_ptr(phys_block_ptr));
	}
			  ...
			  size_t size, uint32_t flags)
{
	...
	va = (__sparse_force uint8_t *)sys_cache_cached_ptr(virt);
	pa = phys;
...
{
	void *va = (__sparse_force void *)sys_cache_cached_ptr(virt);
...
int sys_mm_drv_remap_region(void *virt_old, size_t size,
			    void *virt_new)
{
	void *va_new = (__sparse_force void *)sys_cache_cached_ptr(virt_new);
	void *va_old = (__sparse_force void *)sys_cache_cached_ptr(virt_old);
			   ...
			   uintptr_t phys_new)
{
	...
	virt_new = (__sparse_force void *)sys_cache_cached_ptr(virt_new);
	virt_old = (__sparse_force void *)sys_cache_cached_ptr(virt_old);
...
__imr void adsp_mm_restore_context(void *storage_buffer)
{
	...
	while (phys_addr != 0) {
		uint32_t phys_addr_uncached =
				POINTER_TO_UINT(sys_cache_uncached_ptr(
